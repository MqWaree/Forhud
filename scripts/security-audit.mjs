...
const requiredSecurityText = [
  ...
  ["apps/server/src/file-sharing.ts", "FILE_LINK_SECRET"],
  ["apps/server/src/file-sharing.ts", "timingSafeEqual"],
  ["apps/server/src/file-sharing.ts", '"Content-Disposition"'],
  ["apps/server/src/file-sharing.ts", 'resolve("/var/lib/fgp")'],
  ["deploy-release.ps1", "expected_script_sha"],
  ["deploy/deploy-fgp-release.sh", "openssl rand -hex 32"],
  ["deploy/deploy-fgp-release.sh", "restore_database"],
  ["deploy/deploy-fgp-release.sh", "start_probe_services"],
  ["deploy/restore-fgp-backup.sh", "pkill -KILL -u fgp"],
  ["apps/server/src/index.ts", "FGP_MAINTENANCE_PROBE"],
  [
