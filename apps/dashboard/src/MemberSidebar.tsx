...
export default function MemberSidebar() {
  ...
  return (
    <aside className="member-sidebar" aria-label="Workspace members">
      <header>
        <span>
          <Users /> Members
        </span>
        <small>{members.filter((member) => member.online).length} online</small>
      </header>
      {groups.map(({ rank, members: ranked }) => (
        <section
          className="member-rank-group"
          key={rank.id}
          style={{ "--member-rank": rank.color } as CSSProperties}
        >
          <h3>
            <i />
            {rank.name} — {ranked.length}
          </h3>
          {ranked.map((member) => (
            <article
              className={`member-row ${member.online ? "online" : "offline"}`}
              key={member.id}
            >
