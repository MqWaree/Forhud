...
if (!existsSync(venvPython)) {
  ...
  if (!python) {
    console.error(
      "Python 3.10–3.13 was not found. Install Python, then run this command again.",
    );
    process.exit(1);
  }
...
if (process.env.SCRAPER_SKIP_BROWSER_INSTALL === "true") {
  console.log(
    "Skipping browser installation for the pre-provisioned production runtime.",
  );
} else if (configuredBrowser && existsSync(configuredBrowser)) {
