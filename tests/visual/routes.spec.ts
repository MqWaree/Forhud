...
for (const [name, path, heading] of routes) {
  test(`${name} renders without browser failures`, async ({ page }) => {
    ...
    await expect(page).toHaveScreenshot(`${name}-normal-1440x900.png`);
...
test("login state", async ({ page }) => {
  ...
  await expect(page).toHaveScreenshot("login-normal-1440x900.png");
  expectCleanDiagnostics(diagnostics);
});
...
test("notification dropdown", async ({ page }) => {
  ...
  await expect(page).toHaveScreenshot("notification-dropdown-normal-1440x900.png");
  expectCleanDiagnostics(diagnostics);
});
...
test("Searcher Import Links drawer", async ({ page }) => {
  ...
  await expect(page).toHaveScreenshot("searcher-import-links-drawer-normal-1440x900.png");
  expectCleanDiagnostics(diagnostics);
});
...
test("Leads Kanban direct move controls", async ({ page }) => {
  ...
  await expect(page).toHaveScreenshot(
    "leads-kanban-direct-move-normal-1440x900.png",
  );
  expectCleanDiagnostics(diagnostics);
});
...
test("file-sharing error state", async ({ page }) => {
  ...
  await expect(page).toHaveScreenshot("file-sharing-error-normal-1440x900.png");
  expectCleanDiagnostics(diagnostics);
});
...
test("auth loading state", async ({ page }) => {
  ...
  await expect(page).toHaveScreenshot("auth-loading-normal-1440x900.png");
  expectCleanDiagnostics(diagnostics);
});
