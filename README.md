...
All workspace-owned records carry a workspace ID. Dashboard sessions use opaque HttpOnly, Secure, SameSite cookies with a rolling 30-day lifetime; passwords are required, capped at 200 characters, and use salted `scrypt` hashes. Scanner IDs contain 80 bits of randomness; known legacy/bootstrap values are never accepted for pairing and are replaced automatically before setup or pairing can proceed. Extension bearer tokens are random, stored only as hashes on the server, and can be revoked independently. Rotating a Scanner ID revokes all existing extension tokens.
