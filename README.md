...
The product price scanner keeps each market isolated. Use the built-in Rust NFA
mode, choose **Game accounts**, or choose **Other items** and enter the product
name. Sources, listings, exports, deletion, and statistics are scoped to that
product. Category price cards appear only when at least three comparable
...
BRAVE_SEARCH_CACHE_TTL_MS=900000
BRAVE_SEARCH_CACHE_MAX_PAGES=2000
SCRAPER_MAX_BODY_BYTES=10485760
SCRAPER_GLOBAL_CONCURRENCY=32
SCRAPER_DYNAMIC_CONCURRENCY=3
...
The existing Product Price Scanner includes an internal **LZT Tracker** view for the official LZT Market Rust feed. It uses `GET https://prod-api.lzt.market/steam` with `game[]=252490`, `order_by=pdate_to_down`, `currency=eur`, and excludes brute-force, phishing, and stealer origins. The API token stays in the server environment.

Production configuration belongs in `/etc/fgp/fgp.env`:

```env
LZT_TRACKER_ENABLED=true
LZT_API_BASE_URL=https://prod-api.lzt.market
LZT_API_TOKEN=
LZT_POLL_INTERVAL_MS=3100
LZT_RECONCILIATION_INTERVAL_MS=900000
LZT_MAX_PRICE_USD=20
LZT_NOTIFY_BELOW_USD=5.00
LZT_NOTIFY_HIGH_HOURS_BELOW_USD=6.00
LZT_NOTIFY_HIGH_HOURS_MINIMUM=2000
LZT_TIMEZONE=Europe/Copenhagen
HAZE_LZT_NOTIFICATIONS_ENABLED=false
DISCORD_USER_TOKEN=
HAZE_LZT_CHANNEL_ID=
HAZE_LZT_POLL_MS=1000
```

To send qualifying alerts through the existing Haze account, configure
`DISCORD_USER_TOKEN` and `HAZE_LZT_CHANNEL_ID` only in the VPS environment,
then set `HAZE_LZT_NOTIFICATIONS_ENABLED` to `true`. FGP owns detection and a
durable alert queue; the separate Haze worker renders the compact card and posts
it. Secrets are never exposed to the dashboard or logs. Failed deliveries retry
with bounded backoff, and baseline imports never generate Discord alerts.

`LZT_API_TOKEN` is optional. With no token, FGP uses only publicly accessible
LZT listing pages, polls no faster than once per minute, and records access
challenges instead of attempting to bypass them. Assign the built-in
**LZT Access** workspace rank to members who may open and control this tracker.

Restart the existing service after changing configuration:

```bash
sudo systemctl restart fgp-api.service
sudo systemctl status fgp-api.service --no-pager
curl -fsS http://127.0.0.1:3001/api/health
```

No additional process manager is needed: the existing `fgp-api.service` starts on boot and restarts after a crash. The tracker uses a database lease, so only one application instance polls at a time. First start imports the current page as a quiet historical baseline by default; later items are persisted before their SSE event is sent. A valid Market-scope token is required for a live connection test.
